...
export const connect = function() {
  ...
  const connection: amqp.AMQPClient = amqp.createConnection({
  ...
  connection.on("close", function(hadError: boolean) {
    logger(
      "Connection to AMQP broker was closed.",
  ...
  const subscribeReplyTo = function() {
    const q = connection.queue("", { exclusive: true }, function(
      info: amqp.QueueCallback
    ) {
      replyToQueue = info.name;
      connected = true;
      q.subscribe({ exclusive: true }, function(
        message,
        _headers,
        deliveryInfo,
        _ack
  ...
  const subscribeWorker = function(
    routingKey: string,
    func: Worker,
    ...
    const q = connection.queue(
      ...
      function(_info: amqp.QueueCallback) {
        q.subscribe({ ack: true, prefetchCount: 1 }, function(
          message,
          headers,
          deliveryInfo,
          ack
  ...
  const sendReply = function(
    ...
    if (replyTo && value) {
      ...
      connection.publish(replyTo, value, options, function(
        err?: boolean,
        msg?: string
      ) {
        if (err) {
    ...
    return new Promise((resolve, reject) => {
      const exchange = connection.exchange(
        ...
        function() {
          ...
          exchange.publish(msg.routingKey, msg.data, options, function(
            failed,
            errorMessage
          ) {
            if (failed) {
