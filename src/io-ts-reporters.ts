/**
 * Copied from https://raw.githubusercontent.com/blackdahila/io-ts-reporters/master/src/index.ts until
 * https://github.com/OliverJAsh/io-ts-reporters/pull/14 is merged and published to npm.
 */
...
export const reporter = <T>(validation: t.Validation<T>) =>
  pipe(
    validation,
    fold(errors => array.compact(errors.map(formatValidationError)), () => [])
  );
